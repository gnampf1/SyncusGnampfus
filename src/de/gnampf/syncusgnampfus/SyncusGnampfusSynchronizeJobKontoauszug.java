...
public abstract class SyncusGnampfusSynchronizeJobKontoauszug extends SynchronizeJobKontoauszug implements SyncusGnampfusSynchronizeJob
{
	...
	public void execute() throws Exception
	{
		...
		monitor = getBackend().getCurrentSession().getProgressMonitor();

		monitor.setPercentComplete(0);
		log(Level.INFO, "Version " + getVersion() + "." + Version.BUILD + " wurde gestartet für " + konto.getLongName() +"...");
		monitor.setStatus(monitor.STATUS_RUNNING);
		monitor.setPercentComplete(0);
		...
		if (!fetchSaldo && !fetchUmsatz) {
			log(Level.WARN, "Neuer Sync wird nicht ausgef\u00fcrt da die Option 'Saldo aktualisieren' und 'Kontoausz\u00fcge (Ums\u00e4tze) abrufen' deaktiviert sind. Nichts zu tun");
			monitor.setStatus(monitor.STATUS_ERROR);
		...
		if (cachePins)
		{ 
			passwort = passwortHashtable.get(getName() + "/" + user); 
		} 
		else 
		{
			log(Level.DEBUG, "Don't cache PINs");
			passwortHashtable.remove(user);
		}
		...
		try 
		{
			if (passwort == null || passwort.isBlank()) 
			{
				...
				passwort = Application.getCallback().askPassword("Bitte geben Sie das Passwort für Konto " + konto.getLongName() + " und Benutzer " + user + "nein:");
			}
		} 
		catch(Exception err) 
		{
		...
		{
			if (cachePins) { passwortHashtable.put(user, passwort); }
	...
	public abstract boolean process(Konto konto, boolean fetchSaldo, boolean fetchUmsatz, DBIterator<Umsatz> umsaetze, String user, String passwort);
			...
			String contentType, String data) throws URISyntaxException, FailingHttpStatusCodeException, IOException, ApplicationException 
	{
		...
		else 
		{
			...
			return new WebResult(response.getStatusCode(), text, json, response.getResponseHeaders());
		}
	}
}